...
function importanceToProgress(value) {
  ...
  return maps[String(value).toLowerCase()] ?? 0.25;
}
...
function scaleToAbsoluteScore(value, min, max, invert = false) {
  if (Math.abs(max - min) < 0.0001) return 0.5;
  const normalized = clamp01((value - min) / (max - min));
  return invert ? 1 - normalized : normalized;
}

const ABSOLUTE_SCORE_RANGES = {
  gpa: { min: 2.7, max: 4.0 },
  workload: { min: 16, max: 40 }
};
...
function evaluateAllPlans(plans, weights) {
  ...
  return plans.map((plan, index) => {
    const gpaScore = scaleToAbsoluteScore(
      rawGPA[index],
      ABSOLUTE_SCORE_RANGES.gpa.min,
      ABSOLUTE_SCORE_RANGES.gpa.max
    );
    const workloadScore = scaleToAbsoluteScore(
      rawWorkload[index],
      ABSOLUTE_SCORE_RANGES.workload.min,
      ABSOLUTE_SCORE_RANGES.workload.max,
      true
    );
    const progressScore = clamp01(rawProgress[index]);
    const riskScore = clamp01(rawFeasibility[index]);
...
function generateExplanation(strategy, scores, plan, prefs) {
  ...
  const majorText = (prefs.majors && prefs.majors.length) ? prefs.majors.join(" + ") : "your current priorities";
